...
import org.buffer.android.buffertextinputlayout.util.*;
...
public class BufferTextInputLayout extends LinearLayout {
    ...
    private boolean toogleMobileNo;
    ...
    public void setToogleMobileNo(boolean toogleMobileNo) {
        this.toogleMobileNo = toogleMobileNo;
        recursiveCheckState();
    ...
    boolean toggleDot = false;
    boolean toggleDropdown = false;
    ...
    public BufferTextInputLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        toogleMobileNo = a.getBoolean(
                R.styleable.BufferTextInputLayout_mobileNoEnabled, false);


        boolean toggleEye = a.getBoolean(
                R.styleable.BufferTextInputLayout_passwordToggleEnabled, false);

        setPasswordVisibilityToggleEnabled(toggleEye);

        if(toggleEye) {
            setPasswordVisibilityToggleDrawable(R.drawable.icon_show_password1);
        }else{

            toggleDot = a.getBoolean(
                    R.styleable.BufferTextInputLayout_dotToggleEnabled, false);

            setPasswordVisibilityToggleEnabled(toggleDot);

            if(toggleDot) {
                passwordToggleEnabled = true;
                setPasswordVisibilityToggleDrawable(R.drawable.dot_selector);
            }else{
                toggleDropdown = a.getBoolean(
                        R.styleable.BufferTextInputLayout_dropdownToggleEnabled, false);

                setPasswordVisibilityToggleEnabled(toggleDropdown);

                if(toggleDropdown) {
                    setPasswordVisibilityToggleDrawable(R.drawable.dropdown_selector);
                    passwordToggleEnabled = true;
                }
            }
        }
    ...
    public void addView(View child, int index, final ViewGroup.LayoutParams params) {
        ...
        if (child instanceof EditText) {
            ...
            if(toogleMobileNo) {
                ...
                int dpAsPixels = (int) (95*scale + 0.5f);

//                lp.leftMargin = 100;

                child.setPadding(dpAsPixels, 0, 0, 0);
            ...
            if (enabled) {
                ...
                if (useDefaultColor) {
                    ...
                    errorView.setTextColor(ContextCompat.getColor(
                            getContext(), R.color.design_default_color_primary));
                }
    ...
    private void recursiveCheckState() {
        for (int i = 0, count = inputFrame.getChildCount(); i < count; i++) {
            final View child = inputFrame.getChildAt(i);
    ...
    private void updatePasswordToggleView() {
        ...
        if (shouldShowPasswordIcon()) {
            if (passwordToggleView == null) {
                ...
                if(toggleDot){
                    View v = (View) LayoutInflater.from(getContext())
                            .inflate(R.layout.dot_icon, inputFrame, false);

                    passwordToggleView = (CheckableImageButton) v.findViewById(R.id.text_input_password_toggle);
                    ...
                    inputFrame.addView(v);
                }else if(toggleDropdown){
                    View v = (View) LayoutInflater.from(getContext())
                            .inflate(R.layout.dropdown_icon, inputFrame, false);
                    ...
                    passwordToggleView = (CheckableImageButton) LayoutInflater.from(getContext())
                            .inflate(R.layout.passsword_icon, inputFrame, false);
                    passwordToggleView.setImageDrawable(passwordToggleDrawable);
                    passwordToggleView.setContentDescription(passwordToggleContentDesc);
...
//            passwordToggleView.setChecked(passwordToggledVisible);
            ...
            if(passwordToggledVisible){
                passwordToggleView.setImageResource(R.drawable.icon_hide_password1);
            }else{
                passwordToggleView.setImageResource(R.drawable.icon_show_password1);
            }
...
//                    r, bottom - top - getPaddingBottom());
            ...
            collapsingTextHelper.setCollapsedBounds(rect.left, getPaddingTop(),
                    r, bottom - top - getPaddingBottom());
